...
class Login_Window:
    ...
    def face_recog(self):
        ...
        def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                self.user_Id = id
                # it is forumula which calculate the percentage of face recognize with face data
                confidence = int((100*(1-predict/300)))

                if confidence > 78:                  # set acccording to yourself
                    cv2.putText(img, "Unlocked", (x, y-5),
                                cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), 3)
                    self.flag = 1

                else:
                    cv2.rectangle(img, (x, y), (x+w, y+h), (0, 0, 255), 3)
                    cv2.putText(img, "locked", (x, y-5),
                                cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), 3)
                    self.flag = 0
    ...
    def login(self):
        if self.txtuser.get() == " " or self.txtpass.get() == "":
            ...
            messagebox.showinfo(
                "Success", "Successfully Login", parent=self.root)
            self.face()
            ...
            try:
                ...
                conn = pymysql.connect(
                    host="localhost", port=3306, user="root", password="", database="face_recognition")
                ...
                if row == None:
                    ...
                    open_main = messagebox.askyesno(
                        "Yes No", "Access only admin", parent=self.root)
                    if self.flag == 0:
                        messagebox.showerror(
                            "Error", "Please Detect your face first", parent=self.root)
    ...
    #                                user="root", password="", database="face_recognition")
    #         my_cursor = conn.cursor()

    #         # Get the recognized user ID using the face data
    #         recognized_user_id = id

    #         # Check if the recognized user ID exists in the database
    #         query = "SELECT * FROM login WHERE userid ="+str(id)
    #         my_cursor.execute(query)
    #         row = my_cursor.fetchone()

    #         if row is None:
    #             messagebox.showerror(
    #                 "Error", "Face not recognized", parent=self.root)
    #         else:
    #             # Prompt the user to access admin (or not)
    #             open_main = messagebox.askyesno(
    #                 "Access Confirmation", "Access as admin?", parent=self.root)
    #             if open_main:
    #                 self.face()
    ...
    def reset_pass(self,cnt):
        if self.txt_security.get() == "":
            ...
            row = my_cursor.fetchone()
            if row == None and cnt>0:
                error_message = "Please enter correct details. " + str(cnt) + " attempts left."
                messagebox.showerror(
                    "Error", error_message, parent=self.root2)
            

            elif cnt==0:
                conn.close()
                messagebox.showerror("Error", "Maximum attempts reached. Closing window.", parent=self.root2)
                ...
                conn.close()
                messagebox.showinfo(
                    "Success", "Your password has been reset successfully", parent=self.root2)
                self.root2.destroy()
    ...
    def forget_password(self):
        if self.txtuser.get() == "":
            messagebox.showerror(
                "Error", "Pleas Enter the valid user name", parent=self.root)
        else:
            # conn = mysql.connector.connect(host="sql6.freemysqlhosting.net", user="sql6495753", password="KLdC4PA9zU",
            #                             database="sql6495753", auth_plugin="mysql_native_password")            #please change it accoridng to database
            #my_cursor = conn.cursor()
            ...
            if row == None:
                messagebox.showerror(
                    "Error", "Please Enter valid username", parent=self.root)
            else:
                self.root2 = Toplevel()
                ...
                l = Label(self.root2, text="Forget Password", font=(
                    "times new roman", 25, "bold"), fg="red", bg="white")
                l.place(x=0, y=10, relwidth=1)

                security_q = Label(self.root2, text="Security Question", font=(
                    "times new roman", 12, "bold"), fg="black", bg="white")
                security_q.place(x=50, y=80)

                security_A = Label(self.root2, text="Enter Your School Name", font=(
                    "times new roman", 12, "bold"), fg="black", bg="white")
                security_A.place(x=50, y=130)

                self.txt_security = ttk.Entry(
                    self.root2, font=("times new roman", 12, "bold"))
                ...
                # button
                btn = Button(self.root2, text="Reset Password", command=lambda: self.reset_pass(self.attempts_left), font=(
                    "times new roman", 15, "bold"), fg="white", bg="red", activeforeground="white", activebackground="green")
    ...
    def get_verify(self):
        try:
            ...
            # Fetch the user's face ID from the login table
            my_cursor.execute(
                "SELECT userid FROM login WHERE username=%s", (self.txtuser.get(),))
            user_face_id = my_cursor.fetchone()
            print(user_face_id[0])
            print(self.user_Id)
            ...
            if user_face_id is None:
                ...
                # Replace with your actual method to get recognized user ID
                recognized_face_id = self.user_Id
                # Note the indexing [0] here
                if recognized_face_id == user_face_id[0]:
                    messagebox.showinfo("Verification", "Face Verified!")
                    self.flag=1
                    
                else:
                    self.flag = 0
                    messagebox.showerror(
                        "Verification Failed", "Face does not match the user ID")
